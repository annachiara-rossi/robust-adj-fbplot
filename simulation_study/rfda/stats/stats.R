...
princomp.fData = function( fD, cov_operator = NULL, threshold = 1, components = NULL )
{
  ...
  eigs = geigen( as.matrix( fD$geometry$Mass_matrix %*%
...
eigen.Cov_operator = function( cov_operator, nelements = NULL )
{
  ...
  eigs = geigen( as.matrix( cov_operator$geometry$Mass_matrix %*%
                              cov_operator$coefficients %*%
...
as.fData.fPC = function( fPC )
{
  return( as.fData( coefficients = diag( sqrt( fPC$evalues ) ),
                    geometry = Geometry( as.basis( fPC ), fPC$geometry$space ) ) )
}

plot.fPC = function( fPC, ...  )
{
  plot( as.fData( coefficients = fPC$efunctions,
