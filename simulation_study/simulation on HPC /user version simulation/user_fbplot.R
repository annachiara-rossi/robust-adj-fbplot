...
# in this version, an object of class fData from roahd can be passed
...
source('KFPCA/R/GetMatList.R')
source('KFPCA/R/GCVbw2D.R')
#source('KFPCA/R/GCVbw1D.R')
source('KFPCA/R/GetGCVbw2D.R')
source('KFPCA/R/GetGCVbw1D.R')
source('KFPCA/R/MeanEst.R')
source('KFPCA/R/kernfun.R')
source('KFPCA/R/basicfun.R')
source('KFPCA.R')
...
#----------------------------- fences computation -----------------------------#
# Input: values of the fData object
# Output: envelope, fences and observations outside fences (outliers)

.my_fbplot_fData = function(Data, Depths = 'MBD', Fvalue = 1.5 )
{
                         ...
                         ... )
{
  ...
  if( ! is.list( adjust ) )
  {
    ...
    out = .my_fbplot_fData( Data$values, Depths, Fvalue )
    ...
    out = .my_fbplot_fData( Data$values, Depths, Fvalue = Fvalue  )
  }
...
functional_case <- function(Data, Depths_spec, Cov_estimator, adjust, ...)
{
  ...
  Data_reduced = fData(Data$values, geometry.L2)
...
eigencouples_estimation = function(Data, Cov, L, N){
  ...
  if( is.null(Cov) ){
    ...
    phi <- KFPCA(Lt, Ly, interval, nK = L, bw = 1, 
                        bwK = 0.045, bwmean = 0.03, dataType = "Dense",
                        nRegGrid = length(t$grid), more = FALSE)$FPC_dis
...
KL_data_generation = function(geometry, ecouples, L, N, trial_size){
  ...
  Data_gauss = as.fData(Data_gauss, geometry)
