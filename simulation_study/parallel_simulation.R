...
# I will only try one combination of parameters
...
cov_estimators = c('Ledoit_Wolf','MRCD_0.5','Spherical', 'Median')
...
simulation <- function(n_curves, outlier_share, c, geometry.L2, spectrum, cov_estimators){
...
parallel::clusterExport(cl=cl, varlist=c(ls(),'.my_fbplot_fData'))

res = pbreplicate(n = B, 
                  expr = simulation(n_curves, outlier_share, c, geometry.L2, 
                                    spectrum, cov_estimators), 
                  simplify = TRUE, 
                  cl = cl)

# I get in each entry of the matrix an array containing the metric on the row
# for all the estimators --> I don't know how to transform it as a matrix

# matrix(unlist(res)) puts on columns each list. So the way to obtain the
# estimators on columns and the results from different iterations on rows
# is to do, for each metric of interest, the following:
...
# save results for the current combination of parameters
...
# TODO: after using all parameters' combinations
